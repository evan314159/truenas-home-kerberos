...
The Kerberos security level set on the client mount option (sec=krb5|krb5i|krb5p) needs to match an enabled Kerberos security level on TrueNAS.  The default (none selected) is all Kerberos security levels are allowed. sec=sys is disabled by global option "Require Kerberos fo NFSv4".  For NFS over any shared network, krb5p provides data encryption and ensures integrity.  When access will always be over a VPN (e.g. Tailscale), krb5i provides integrity only, leaving encryption to the VPN.
...
## Backup and Recovery
I am snapshotting the Kerberos apps/krb5kdc/data directory, and then backing up the snapshots as per usual. Rollbacks will cause issues if the Kerberos database has changed between snapshots, e.g. due to key changes.  The Kerberos server can be reset by deleting the contents of the data dataset and restarting the container.
